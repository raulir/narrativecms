function cms_input_xy_layout($root){

	var $scope = $root ? $root.find('.cms_input_xy_container') : $('.cms_input_xy_container');
...
function cms_input_xy_init($root){

	cms_input_xy_layout($root);

	var $scope = $root ? $root.find('.cms_input_xy_container') : $('.cms_input_xy_container');

	$scope.not('.cms_input_xy_ok').each(function(){

		var $container = $(this);

		$container.addClass('cms_input_xy_ok');

		$('.cms_input_xy_set_button', $container).on('click.cms', function(){
			$container.addClass('cms_input_xy_active')
			cms_input_xy_picker($container)
		})

		$('.cms_input_xy_clear', $container).on('click.cms', function(){
			$('.cms_input_xy_x', $container).val('')
			$('.cms_input_xy_y', $container).val('')
			$('.cms_input_xy_pointer', $container).css({'opacity':''})
		})

		$('.cms_input_xy_x', $container).on('change.cms keyup.cms', function(){
			cms_input_xy_layout($container)
		})

		$('.cms_input_xy_y', $container).on('change.cms keyup.cms', function(){
			cms_input_xy_layout($container)
		})
