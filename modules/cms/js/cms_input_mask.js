function cms_input_mask_layout($root){

	var $scope = $root ? $root.find('.cms_input_mask_container') : $('.cms_input_mask_container');

	$scope.each(function(){

		var $container = $(this);

		$('.cms_input_mask_image_inner,.cms_input_mask_image_value', $container).each(function(){
		...
		cms_input_mask_display($container)

	})

}

function cms_input_mask_init($root){

	cms_input_mask_layout($root);

	var $scope = $root ? $root.find('.cms_input_mask_container') : $('.cms_input_mask_container');

	$scope.not('.cms_input_mask_ok').each(function(){

		var $container = $(this);

		$container.addClass('cms_input_mask_ok');

		$('.cms_input_mask_set_button', $container).on('click.cms', function(){
			$container.addClass('cms_input_mask_active')
			cms_input_mask_picker($container)
		})

		$('.cms_input_mask_clear', $container).on('click.cms', function(){
			$('.cms_input_mask_value', $container).val('')
			$('.cms_input_mask_image_value', $container).remove()
		})
