<div class="cms_input cms_input_select cms_input_list <?= !empty($mandatory_class) ? $mandatory_class : '' ?>
	...
	<select class="cms_input_select_select <?= !empty($name) ? $name : '' ?>"
		...
		<?php foreach($values as $key => $key_label): ?>
			<option value="<?= $key ?>"<?= (string)$key === (string)$value ? ' selected="selected"' : '' ?>><?= $key_label ?></option>
		<?php endforeach ?>
