	...
	<?php if (!(empty($pages) || count($pages) == 0)): ?>
		<ul id="pages_list" class="admin_list_sortable">
			<?php foreach($pages as $page): ?>
				<li class="cms_list_sortable_item <?= !empty($page['status']) ? ' cms_item_hidden ' : '' ?>" style="background-image: url('<?php print($GLOBALS['config']['base_url']); ?>modules/cms/img/drag.png'); ">
