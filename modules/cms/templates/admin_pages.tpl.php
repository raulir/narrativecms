	...
	<?php if (!(empty($pages) || count($pages) == 0)): ?>
		<ul id="pages_list" class="admin_list_sortable">
			<?php foreach($pages as $page): ?>
				<li style="background-image: url('<?php print($GLOBALS['config']['base_url']); ?>modules/cms/img/drag.png'); ">
					<input type="hidden" class="page_id" value="<?php print($page['page_id']); ?>">
					<div class="admin_text"><?php print(!empty($page['title']) ? $page['title'] : '[ no title ]'); ?></div>
